...
int num_first_hidden;  
int num_second_hidden;  
...
int main(void) {
    ...
    bool model_selection = false; // randomly select one base model to execute
    int model_index; // The index of selected base model 
    //Variables on predictions
    double train_accuracy;
    double test_accuracy;
    double current_train_accuracy = -INFINITY;

    for (int i=0;i<MAX_NUM_MODEL;i++){
        if(model_selection == false){
           // srand(time(NULL));
            model_index = rand() % MAX_NUM_MODEL;
            num_first_hidden = model_one_nodes[0];
            num_second_hidden = model_one_nodes[1];
        }
        else{
            model_index = (MAX_NUM_MODEL-1)- model_index;
            num_first_hidden = model_second_nodes[0];
            num_second_hidden = model_second_nodes[1];
        }
        model_selection = true;

       // Create array structures for the selected base model
       ...
       //Initialize weights
       init_weights(num_first_hidden,num_second_hidden,num_output,
                    input_weights,hidden_weights,output_weights,
                    bias_weights1,bias_weights2);
       //Training
       for (int i=0; i < MAX_EPOCHS; i++){
           epoch(num_first_hidden, num_second_hidden,num_output,
                 input_weights,hidden_weights,output_weights,
                 bias_weights1,bias_weights2,
        ...
        train_accuracy = accuracy(NUM_TRAIN, training_data,training_labels,
                                  num_first_hidden,num_second_hidden,num_output,
                                  input_weights,hidden_weights,output_weights,
                                  bias_weights1,bias_weights2,
                                  hidden_layers_first,hidden_layers_second,hidden_nets_first,hidden_nets_second,
                                  output_layer,output_nets);
